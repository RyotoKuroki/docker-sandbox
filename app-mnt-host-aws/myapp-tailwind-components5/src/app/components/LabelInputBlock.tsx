...
const LabelInputBlock = forwardRef<HTMLDivElement, ComposableComponentProps>(({
    ...
    return (
        <div className={cn(
            "flex flex-row border-b items-center h-full", // デフォルトのスタイル：縦並び、要素間に間隔
            className // 追加のクラス名を適用
        )}>
            {children}
        </div>
...
const LabelBlock = forwardRef<HTMLDivElement, ComposableComponentProps>(({
    ...
    return (
        <div className={cn(
            "bg-blue-100 border-r w-[200px] h-full p-1 flex items-center", // デフォルトのスタイル：縦並び、要素間に間隔
            className // 追加のクラス名を適用
        )}>
            {children}
        </div>
...
const InputBlock = forwardRef<HTMLDivElement, ComposableComponentProps>(({
    ...
    return (
        <div className={cn(
            "flex-1 h-full p-1 flex items-center", // デフォルトのスタイル：縦並び、要素間に間隔
            className // 追加のクラス名を適用
        )}>
            {children}
        </div>
...
const LabelBlockNoBG = forwardRef<HTMLDivElement, ComposableComponentProps>(({
    ...
    return (
        <div className={cn(
            "w-[200px] h-full p-1 flex justify-end items-center", // デフォルトのスタイル：縦並び、要素間に間隔
            className // 追加のクラス名を適用
        )}>
            {children}
        </div>
