import { z, ZodString } from "zod";

export enum ValidatiionSideEnum {
  CLIENT = "CLIENT",
  SERVER = "SERVER",
  HOGE_1 = "HOGE_1",
  HOGE_2 = "HOGE_2",
}

const validEmail = (zStr: ZodString): z.ZodString => {
  return zStr.email({ message: "有効なメールアドレスを入力するっす。" });
};

const validNickName = (zStr: ZodString): z.ZodString => {
  return zStr.nonempty({ message: "ひっす入力っす！" }).max(10, `[validNickName] 最大10桁っす`);
};

const validSubsPriority = (): z.ZodNumber => {
  return z.coerce
    .number()
    .min(1, { message: "1以上で入力するっす" })
    .max(100, { message: "100以下で入力するっす" });
};

// schema of sub-email.
const inDtoSubsSaveSomeSchema = z.object({
  priority: validSubsPriority(),
  subEmail: z.string().email({ message: "有効なメールアドレスを入力してください。" }),
});
// schema of this page form.
const inDtoSaveSomeSchema = z.object({
  name: z.string().nonempty({ message: "名前を入力してください！" }),
  email: validEmail(z.string()),
  nickname: validNickName(z.string()),
  validSide: z.union([
    z.literal(ValidatiionSideEnum.CLIENT),
    z.literal(ValidatiionSideEnum.SERVER),
    z.literal(ValidatiionSideEnum.HOGE_1),
    z.literal(ValidatiionSideEnum.HOGE_2),
  ]),
  // age: z.number().min(18, { message: '18歳以上である必要があります。' }).optional(),
  subs: z.array(inDtoSubsSaveSomeSchema).min(1, {
    message: "少なくとも1つのアイテムを追加してください",
  }),
});

// generate zod-object -> type
type inDtoSaveSomeType = z.infer<typeof inDtoSaveSomeSchema>;
// generate zod-object -> type of errors
type inDtoSaveSomeErrorType = z.inferFlattenedErrors<typeof inDtoSaveSomeSchema>["fieldErrors"];

const outDtoSchema = z.object({
  msg: z.string(),
});
type outDtoType = z.infer<typeof outDtoSchema>;
type outDtoErrorType = z.inferFlattenedErrors<typeof outDtoSchema>["fieldErrors"];

export {
  inDtoSaveSomeSchema,
  type inDtoSaveSomeType,
  type inDtoSaveSomeErrorType,
  outDtoSchema,
  type outDtoType,
  type outDtoErrorType,
};
