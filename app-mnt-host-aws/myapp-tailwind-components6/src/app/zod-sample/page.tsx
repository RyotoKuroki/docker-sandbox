...
export default function HomePage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    switch (formData.validSide) {
      case ValidatiionSideEnum.CLIENT:
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center p-4">
      ...
      <div className="w-full max-w-md bg-white p-8 rounded-lg shadow-md">
        <h1 className="text-2xl font-bold text-center text-gray-700 mb-6">お問い合わせフォーム</h1>

        <p className="whitespace-pre-wrap py-5">{cautionName}</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            />
            {errors.nickname && <p className="mt-1 text-xs text-red-500">{errors.nickname[0]}</p>}
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Subメールアドレス</label>
              ...
              <div className="flex flex-row justify-start items-start gap-2" key={index}>
                <div className="w-[30%] items-start">
                  ...
                  {proxyForm.formState.errors.subs?.[index]?.priority && (
                    <p
                      id={`${uiItemNames.emailList}-${index}-${uiItemNames.priority}-${uiItemNames.error}`}
                ...
                <div className="flex-1 items-start">
                  ...
                  {proxyForm.formState.errors.subs?.[index]?.subEmail && (
                    <p
                      id={`${uiItemNames.emailList}-${index}-${uiItemNames.subEmail}-${uiItemNames.error}`}
                      className="mt-1 text-xs text-red-500"
                    >
          ...
          <div className="flex flex-row gap-5">
            ...
            <div className="flex flex-row gap-3">
              <label>
                <input
                  type="radio"
                  value={ValidatiionSideEnum.CLIENT}
          ...
          <div className="w-full text-center">
            <Link href="http://localhost:1080" target="_blank" className="text-blue-500">
              <u>maildev</u>
            </Link>
          </div>
        </form>
